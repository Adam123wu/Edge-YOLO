...
def main():
    ...
    scheduler = ReduceLROnPlateau(optimizer, mode="min", factor=0.1, patience=15, verbose=True, threshold=0.00001,
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (images, boxes, labels, _) in enumerate(train_loader):
        ...
        # break #test
        global now
        with open("loss.txt", "a") as fp:
            fp.write(f"{now},{losses.val}\n")
        print(f"{now}:{losses.val}")
        now += 1
